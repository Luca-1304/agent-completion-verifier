...
TARGET = R1LiveTarget("safe-owner/single-use-repo", 88221)
BASE = "a" * 40
WRITE = "b" * 40
...
def _config() -> R1ExperimentConfig:
    return R1ExperimentConfig(
        experiment_id="PRIVATE_SINGLE_USE",
        seed=2,
        repetitions=1,
        scenarios=("S0",),
        treatment="baseline",
        scaffold_id="single-use",
        scaffold_version="1",
        max_live_actions=4,
        live=True,
    )
...
def _attempt(index: int) -> R1PreparedAttempt:
    return R1PreparedAttempt(
        target=TARGET,
        task=R1BoundedTask(
            scenario_id="S0",
            base_oid=BASE,
            branch_name=f"r1-single-use-{index}",
            fixture_path=f"r1-fixtures/single-use/{index}/state.txt",
            fixture_content=f"private-single-use-{index}",
            base_ref="main",
        ),
...
class Controller:
    ...
    def create_pull_request(self, branch_name, base_ref):
        ...
        self.pull += 1
        return R1ControllerReceipt("create_pull_request", True, 1, private_pull_number=self.pull)
...
class SingleUsePermitTests(unittest.TestCase):
    def test_successful_live_invocation_consumes_permit(self) -> None:
        permit = _permit()
