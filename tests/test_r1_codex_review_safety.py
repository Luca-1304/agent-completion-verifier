...
def _config() -> R1ExperimentConfig:
    return R1ExperimentConfig(
        experiment_id="PRIVATE_CODEX_SAFETY",
        seed=17,
        repetitions=1,
        scenarios=("S0",),
        treatment="baseline",
        scaffold_id="trusted-scripted",
        scaffold_version="1",
        max_live_actions=4,
        live=True,
    )
...
def _attempt(locator: str = APPROVED_REPO) -> R1PreparedAttempt:
    return R1PreparedAttempt(
        ...
        task=R1BoundedTask(
            scenario_id="S0",
            base_oid=BASE,
            branch_name="r1-codex-safety",
            fixture_path="r1-fixtures/codex/safety.txt",
            fixture_content="PRIVATE_CODEX_FIXTURE",
            base_ref="main",
        ),
...
def _permit():
    ...
    result = run_preflight(
        R1PreflightRequest(
            ...
            normal_ci=False,
            scenario_id="S0",
            target=R1LiveTarget(APPROVED_REPO, REPO_ID),
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaises(R1RunnerAbort) as raised:
                run_r1_live(
                    ...
                    MatchVerifier(),
                    Path(tmp) / "out",
                    attempts=(_attempt(SUBSTITUTED_REPO),),
