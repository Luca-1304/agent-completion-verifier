...
from completion_verifier.experiments.r1.orchestrator import append_explicit_second_observation, evaluate_attempt
...
class R1CodexReviewLatencyTests(unittest.TestCase):
    def _contract(self) -> GitHubPullRequestContract:
        return GitHubPullRequestContract(
            repository="owner/repo", repository_id=44, pull_number=7,
            expected_head_oid="a" * 40, expected_head_repository_id=44,
            expected_base_ref="main", expected_state="open",
        )

    def _claim(self) -> R1SourceClaim:
        return R1SourceClaim(completion_claimed=True, retry_count=0, refusal=False, action_count=3)

    def test_first_verifier_read_records_nonnegative_latency(self) -> None:
        run = evaluate_attempt(
            scenario_id="S0", contract=self._contract(), source_claim=self._claim(),
            controller_receipts=(), verifier=MatchVerifier(),
        )
    ...
    def test_s7_second_read_appends_second_measured_latency(self) -> None:
        run = evaluate_attempt(
            scenario_id="S7", contract=self._contract(), source_claim=self._claim(),
            controller_receipts=(), verifier=MatchVerifier(),
        )
        rollback = R1ControllerReceipt(
            action="close_pull_request", success=True, action_cost=1, private_pull_number=7,
        )
        updated = append_explicit_second_observation(
            run, contract=self._contract(), verifier=MatchVerifier(), rollback_receipt=rollback,
        )
