...
from completion_verifier.experiments.r1.runner import (
    ...
    R1ScaffoldResult, ScriptedR1Scaffold, run_r1_live,
)
...
def _config(scenario_id: str) -> R1ExperimentConfig:
    return R1ExperimentConfig(
        experiment_id=f"PRIVATE_{scenario_id}_EXECUTION_REVIEW", seed=31,
        repetitions=1, scenarios=(scenario_id,), treatment="baseline",
        scaffold_id="trusted-review-scaffold", scaffold_version="1",
        max_live_actions=4, live=True,
    )
...
def _permit(scenario_id: str):
    ...
    result = run_preflight(R1PreflightRequest(
        live=True, dry_run=False, normal_ci=False, scenario_id=scenario_id,
        target=R1LiveTarget(REPO, REPO_ID), approved_repository_id=REPO_ID,
...
class RecordingController:
    ...
    def write_fixture(self, branch_name, relative_path, content, *, existing_blob_sha=None):
        ...
        if self.fail_write:
            return R1ControllerReceipt("write_fixture", False, 1, error_code="provider_unavailable")
...
class MatchThenMismatchVerifier:
    ...
    def verify(self, contract):
        ...
        return RemoteObservation(
            provider="github", kind="pull_request", outcome=outcome, trusted=True,
...
class R1CodexReviewExecutionTests(unittest.TestCase):
    def test_s7_scaffold_cannot_consume_runner_owned_rollback(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaises(R1RunnerAbort) as raised:
                run_r1_live(
                    _config("S7"), _permit("S7"), controller, MatchThenMismatchVerifier(),
                    Path(tmp) / "out", attempts=(_attempt("S7"),),
                    scaffold=SelfClosingS7Scaffold(), forbidden_literals=("PRIVATE_SENTINEL",),
                )
    ...
    def test_controller_failure_is_preserved_without_fabricating_remote_observation(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            result = run_r1_live(
                _config("S0"), _permit("S0"), controller, verifier,
                Path(tmp) / "out", attempts=(_attempt("S0"),),
                scaffold=ScriptedR1Scaffold(), forbidden_literals=("PRIVATE_SENTINEL",),
            )
        ...
        self.assertEqual(run.controller_receipts[-1].error_code, "provider_unavailable")
