...
The source-controlled harness, dry controller, preflight gate, narrow GitHub writer, scenario definitions, artifact pipeline, metrics and runner are implemented on the R1 development branch and tested with fake transports/verifiers. A live pilot is still gated separately.
...
The live runner also checks that the writer is bound to the same disposable target before any mutation. It reserves a cleanup action before pull-request creation so an agent cannot consume the final action while leaving a research PR open.
...
Controller failures are recorded separately from verifier outcomes. Cleanup failure does not retroactively alter the earlier remote observation.
...
A source-controlled sentinel scan is applied before durable public artifacts are written, and the manifest detects changed, missing and untracked files.
